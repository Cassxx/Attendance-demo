                    ...
                    query.findObjects(new FindListener<SignOut>() {
                        ...
                        public void done(List<SignOut> xlist, BmobException e) {
                            if (e==null){
                                ...
                                for (int k=0;k<signListByMonthListCopy.size();k++){
                                    for (int j=0;j<signListByMonthListCopy2.size();j++){
                                        if (signListByMonthListCopy.get(k).getSignDate().equals(signListByMonthListCopy2.get(j).getSignDate())) {
                                            if (signListByMonthListCopy.get(k).getSignStatus().equals(signListByMonthListCopy2.get(j).getSignStatus())) {
                                                signListByMonthListCopy.get(k).setSignStatus(1);
                                                DateToCalender(signListByMonthListCopy);
                                            } else if (signListByMonthListCopy.get(k).getSignStatus().equals(3) || signListByMonthListCopy.get(k).getSignStatus().equals(5)) {
                                                DateToCalender(signListByMonthListCopy);
                                            } else {
                                                signListByMonthListCopy.get(k).setSignStatus(2);
                                                DateToCalender(signListByMonthListCopy);
                                            }
                                        }
                                    }
